...
public class MiscMath {
    ...
    public static double[] getWorldCoords(int onscreen_x, int onscreen_y) {
        return new double[]{((onscreen_x - (Window.getWidth() / 2)) / Camera.getZoom()) + Camera.getX(),
                ((onscreen_y - (Window.getHeight() / 2)) / Camera.getZoom()) + Camera.getY()};
    }

    public static int[] getOnscreenCoords(double world_x, double world_y) {
        return new int[]{(int) round((world_x - Camera.getX()) * Camera.getZoom(), 1) + (Window.getWidth() / 2),
                (int) round((world_y - Camera.getY()) * Camera.getZoom(), 1) + (Window.getHeight() / 2)};
    }
